...
BOOST_AUTO_TEST_CASE(convert_ints) { BOOST_CHECK_EQUAL(gtfs::toInt("123456789"), 123456789); }
...
BOOST_AUTO_TEST_CASE(not_in_header)
{
    ...
    BOOST_CHECK_EQUAL("", gtfs::construct<std::string>("fourth", gtfs::forward, header, data));
    auto const one =
        gtfs::construct_as_optional<std::string, false>("empty", gtfs::forward, header, data);
    BOOST_CHECK_EQUAL(boost::none, one);
    auto const two =
        gtfs::construct_as_optional<std::string, true>("empty", gtfs::forward, header, data);
    auto comp = boost::optional<std::string>("");
    BOOST_CHECK_EQUAL(comp, two);
    auto const three =
        gtfs::construct_as_optional<std::uint64_t, false>("sixth", gtfs::toInt, header, data);
    BOOST_CHECK_EQUAL(boost::none, three);
    auto const four =
        gtfs::construct_as_optional<std::uint64_t, false>("seventh", gtfs::toInt, header, data);
