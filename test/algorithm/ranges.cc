...
BOOST_AUTO_TEST_CASE(on_alternating_ranges)
{
    ...
    algorithm::on_alternating_ranges(data.begin(), data.end(), even, add_to_results);
    BOOST_CHECK_EQUAL(result.size(), 4ull);
    ...
    algorithm::on_alternating_ranges(data.begin(), data.begin(), even, add_to_results);
    BOOST_CHECK_EQUAL(result.size(), 4ull);
}
...
BOOST_AUTO_TEST_CASE(on_ranges)
{
    ...
    nepomuk::algorithm::on_ranges(data.begin(), data.end(), even, add_to_results);
