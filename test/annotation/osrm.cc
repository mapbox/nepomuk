...
BOOST_AUTO_TEST_CASE(annotate_osrm_trip_from_coordinates)
{
    ...
    std::transform(from.begin(), from.end(), std::back_inserter(origin), make_converter(from_coordinate));
    std::transform(to.begin(), to.end(), std::back_inserter(destination), make_converter(to_coordinate));
    ...
    if (trip && dtrip)
    {
        ...
        BOOST_CHECK_EQUAL(OSRM_ANNOTATED_QUERY_A_J, annotated_trip);
        BOOST_CHECK_EQUAL(OSRM_ANNOTATED_QUERY_A_J, annotated_dtrip);
    }
}
